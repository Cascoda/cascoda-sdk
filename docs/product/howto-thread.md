...
- [Getting Started with Thread](#getting-started-with-thread)
	...
	- [Step 2 - Access the Hub](#step-2---access-the-hub)
...
The [Cascoda Thread Evaluation Kit](https://www.cascoda.com/wheretobuy/) contains most of the required parts for evaluating Thread functionality. In addition, a Cascoda KNX IoT Hub is required.
...
The KNX IoT Hub is capable of routing IPv6 traffic between the Thread Network and the adjacent
...
For more information about the Hub router, see the [Setup Guide.](../dev/hub-router-setup.md)
...
If the Hub has been configured correctly, there will also be a global IPv6 address in that list, that can be routed off mesh.
...
You should also be able to ping from the KNX IoT Hub. 
