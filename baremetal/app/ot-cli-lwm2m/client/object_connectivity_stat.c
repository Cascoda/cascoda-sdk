 ...
 *  StartOrReset         |  6 |   E   | Single|  Yes |         |       |         |             |
 *  Stop                 |  7 |   E   | Single|  Yes |         |       |         |             |
 *  Collection Period    |  8 |   RW  | Single|  No  | Integer |       | Seconds |             |
 */
...
static uint8_t prv_read(uint16_t instanceId, int *numDataP, lwm2m_data_t **dataArrayP, lwm2m_object_t *objectP)
{
	...
	if (*numDataP == 0)
	{
		uint16_t resList[] = {RES_O_TX_DATA, RES_O_RX_DATA, RES_O_MAX_MESSAGE_SIZE, RES_O_AVERAGE_MESSAGE_SIZE};
...
static void prv_stopCollection(lwm2m_object_t *objectP)
{
	conn_s_data_t *myData      = (conn_s_data_t *)objectP->userData;
