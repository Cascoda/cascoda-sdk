 ...
 *    Ciaran Woodward, Cascoda Ltd. - Please refer to git log
 *    
 *******************************************************************************/
	...
	do
	{
		...
		case LWM2M_SECURITY_SECRET_KEY_ID:
			...
			if (targetP->secretKey != NULL)
			{
				...
				memcpy(
				    targetP->secretKey, (char *)dataArray[i].value.asBuffer.buffer, dataArray[i].value.asBuffer.length);
		...
		//Optional, ignore (This object can only be written during bootstrap, where unimplemented optionals can be ignored)
...
const char *security_get_public_id(lwm2m_object_t *objectP, uint16_t secObjInstID, size_t *idLen)
{
	...
	if (NULL != targetP)
	{
		*idLen = (size_t)targetP->publicIdLen;
		return targetP->publicIdentity;
	}
...
const char *security_get_secret_key(lwm2m_object_t *objectP, uint16_t secObjInstID, size_t *keyLen)
{
	...
	if (NULL != targetP)
	{
		*keyLen = (size_t)targetP->secretKeyLen;
		return targetP->secretKey;
	}
