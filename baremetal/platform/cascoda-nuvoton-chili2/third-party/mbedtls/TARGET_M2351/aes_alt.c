                            ...
                            size_t               dataSize)
{
	...
	assert((dataSize % 16 == 0) && (dataSize <= MAX_DMA_CHAIN_SIZE));
	...
	AES_Open(CRPT_dyn, 0, ctx->encDec, ctx->opMode, ctx->keySize, AES_IN_OUT_SWAP);
	AES_SetInitVect(CRPT_dyn, 0, ctx->iv);
	AES_SetKey(CRPT_dyn, 0, ctx->keys, ctx->keySize);
	...
	assert(!crypto_dma_buffs_overlap(pIn, dataSize, pOut, dataSize));

	AES_SetDMATransfer(CRPT_dyn, 0, (uint32_t)pIn, (uint32_t)pOut, dataSize);

	crypto_aes_prestart();
	AES_Start(CRPT_dyn, 0, CRYPTO_DMA_ONE_SHOT);
	crypto_aes_wait();
	...
	ctx->iv[0] = CRPT_dyn->AES_FDBCK[0];
	ctx->iv[1] = CRPT_dyn->AES_FDBCK[1];
	ctx->iv[2] = CRPT_dyn->AES_FDBCK[2];
	ctx->iv[3] = CRPT_dyn->AES_FDBCK[3];

	/* Disable AES interrupt */
	AES_DISABLE_INT(CRPT_dyn);
