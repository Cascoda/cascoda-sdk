 ...
 *                  1 if normalized
 */
#define INTERNAL_MPI_IS_NORM(N, P) ((mbedtls_mpi_cmp_int(&N, 0) >= 0) && (mbedtls_mpi_cmp_mpi(&N, &P) < 0))
...
int mbedtls_internal_ecp_init(const mbedtls_ecp_group *grp)
{
	...
	/* Acquire ownership of ECC accelerator (works recursively, returns 0 if first acquisition)*/
	if (crypto_ecc_init())
		return 0;
...
{
	(void)grp;
	/* Release ownership of ECC accelerator (works recursively, returns 0 if final release)*/
	if (crypto_ecc_deinit())
		return;
}
...
int mbedtls_internal_ecp_normalize_jac(const mbedtls_ecp_group *grp, mbedtls_ecp_point *pt)
{
     ...
     * 0 <= y < P
     */
	if (mbedtls_mpi_cmp_int(&pt->Z, 1) == 0 && mbedtls_mpi_cmp_int(&pt->X, 0) >= 0 &&
	    mbedtls_mpi_cmp_mpi(&pt->X, &grp->P) < 0 && mbedtls_mpi_cmp_int(&pt->Y, 0) >= 0 &&
                                 ...
                                 uint32_t                       eccop)
{
	...
	MBEDTLS_MPI_CHK(internal_mpi_write_eccreg(Np, (uint32_t *)CRPT_dyn->ECC_A, NU_ECC_BIGNUM_MAXWORD));
	INTERNAL_MPI_NORM(&Np, grp->B, N_, grp->P);
	MBEDTLS_MPI_CHK(internal_mpi_write_eccreg(Np, (uint32_t *)CRPT_dyn->ECC_B, NU_ECC_BIGNUM_MAXWORD));

	/* Configure ECC prime modulus */
	MBEDTLS_MPI_CHK(internal_mpi_write_eccreg(&grp->P, (uint32_t *)CRPT_dyn->ECC_N, NU_ECC_BIGNUM_MAXWORD));
	...
	{
		INTERNAL_MPI_NORM(&Np, *m, N_, grp->N);
		MBEDTLS_MPI_CHK(internal_mpi_write_eccreg(Np, (uint32_t *)CRPT_dyn->ECC_K, NU_ECC_BIGNUM_MAXWORD));
	}

	/* Configure ECC point (X1, Y1) */
	INTERNAL_MPI_NORM(&Np, P->X, N_, grp->P);
	MBEDTLS_MPI_CHK(internal_mpi_write_eccreg(Np, (uint32_t *)CRPT_dyn->ECC_X1, NU_ECC_BIGNUM_MAXWORD));
	INTERNAL_MPI_NORM(&Np, P->Y, N_, grp->P);
	MBEDTLS_MPI_CHK(internal_mpi_write_eccreg(Np, (uint32_t *)CRPT_dyn->ECC_Y1, NU_ECC_BIGNUM_MAXWORD));

	/* Configure ECC points (X2, Y2) */
	if (eccop == ECCOP_POINT_ADD)
	{
		INTERNAL_MPI_NORM(&Np, Q->X, N_, grp->P);
		MBEDTLS_MPI_CHK(internal_mpi_write_eccreg(Np, (uint32_t *)CRPT_dyn->ECC_X2, NU_ECC_BIGNUM_MAXWORD));
		INTERNAL_MPI_NORM(&Np, Q->Y, N_, grp->P);
		MBEDTLS_MPI_CHK(internal_mpi_write_eccreg(Np, (uint32_t *)CRPT_dyn->ECC_Y2, NU_ECC_BIGNUM_MAXWORD));
	}

	crypto_ecc_prestart();
	CRPT_dyn->ECC_CTL =
	    (grp->pbits << CRPT_ECC_CTL_CURVEM_Pos) | eccop | CRPT_ECC_CTL_FSEL_Msk | CRPT_ECC_CTL_START_Msk;
	...
	MBEDTLS_MPI_CHK(internal_mpi_read_eccreg(&R->Y, (uint32_t *)CRPT_dyn->ECC_Y1, NU_ECC_BIGNUM_MAXWORD));
                                 ...
                                 uint32_t           modop)
{
	...
	if (mbedtls_mpi_cmp_int(o1, 0) < 0 || mbedtls_mpi_cmp_int(o2, 0) < 0)
	{
	...
	if (modop != MODOP_DIV && modop != MODOP_MUL && modop != MODOP_ADD && modop != MODOP_SUB)
	{
		return MBEDTLS_ERR_ECP_BAD_INPUT_DATA;
	}
	...
	if (modop == MODOP_MUL || modop == MODOP_ADD || modop == MODOP_SUB)
	{
	...
	MBEDTLS_MPI_CHK(internal_mpi_write_eccreg(p, (uint32_t *)CRPT_dyn->ECC_N, NU_ECC_BIGNUM_MAXWORD));

	crypto_ecc_prestart();
	CRPT_dyn->ECC_CTL =
	    (pbits << CRPT_ECC_CTL_CURVEM_Pos) | (ECCOP_MODULE | modop) | CRPT_ECC_CTL_FSEL_Msk | CRPT_ECC_CTL_START_Msk;
	ecc_done = crypto_ecc_wait();
